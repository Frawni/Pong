...
#        self.score = 0
    ...
    def pause(self):
        if not self.speeds_bkp:
            # save
            self.speeds_bkp["paddle1"] = self.player1.speed
            self.speeds_bkp["paddle2"] = self.player2.speed
            self.speeds_bkp["ball"] = (self.ball.x_change, self.ball.y_change)

            # set to 0
            self.player1.speed = 0
            self.player2.speed = 0
            self.ball.x_change, self.ball.y_change = 0, 0

            self.ball.colour = (0xff, 0x00, 0x69)
        else:
            # restore
            self.player1.speed = self.speeds_bkp["paddle1"]
            self.player2.speed = self.speeds_bkp["paddle2"]
            self.ball.x_change, self.ball.y_change = self.speeds_bkp["ball"]

            # flush bkp
            self.speeds_bkp = {}

            self.ball.colour = (0, 0, 0)
